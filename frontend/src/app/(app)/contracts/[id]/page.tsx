...
import type { ContractDetail, Clause, ClauseDetail } from "@/types";
...
export default function ContractDetailPage() {
  ...
  const fetchClauses = useCallback(async () => {
    try {
      const { data } = await api.get<Clause[]>(
  ...
  useEffect(() => {
    Promise.all([fetchContract(), fetchClauses()]).finally(() =>
      setLoading(false)
    );
  }, [fetchContract, fetchClauses]);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">
            ...
            {formatDate(contract.created_at)} • {statusLabel(contract.status)}
          </p>
        </div>
      </div>
